...
class BundleProducts extends FormWidgetBase
{
    protected $defaultAlias = 'bundleProducts';
...
//        $this->addJs('script.js');
        ...
        $listConfig = \Yaml::parseFile(plugins_path(
            'qcsoft/app/formwidgets/bundleproducts/available-items-list.yaml'
        ));

        $listConfig['recordOnClick'] = 'vbus.app["' . $this->getId() . '"].$refs.bundleProducts.onSelectProduct(:id)';
...
//            $query->with(['catalogitem', 'catalogitem.main_image', 'catalogitem.main_category']);
        ...
        $this->model->bindEvent('model.afterSave', function ()
        {
            if ($bundleRequest = \Request::input($this->parentForm->arrayName))
            {
                ...
                {
                    $this->saveBundleProductsToRelation($widgetSaveData->items);
                }
            }
        });
    }
    ...
    protected function saveBundleProductsToRelation($requestedItems)
    {
